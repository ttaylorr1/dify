...
class SandboxMessagesCleanService:
    ...
    def clean_sandbox_messages_by_time_range(
        ...
        logger.info(
            ...
            start_from, end_before, batch_size
        )
    ...
    def _clean_sandbox_messages_by_time_range(
        ...
        while True:
            ...
            with Session(db.engine, expire_on_commit=False) as session:
                ...
                if not messages:
                    logger.info(
                        "clean_messages (batch %s): no more messages to process", stats["batches"]
                    )
                    break
                ...
                if not app_ids:
                    logger.info(
                        "clean_messages (batch %s): no app_ids found, skip", stats["batches"]
                    )
                    continue
            ...
            if not apps:
                logger.info(
                    "clean_messages (batch %s): no apps found, skip", stats["batches"]
                )
                continue
            ...
            if not sandbox_message_ids:
                logger.info(
                    "clean_messages (batch %s): no sandbox messages found, skip",
                    stats["batches"]
                )
                continue
    ...
    def _batch_fetch_tenant_plans(cls, tenant_ids: Sequence[str]) -> dict[str, SubscriptionPlan]:
        ...
        redis_keys = [
            f"{cls.PLAN_CACHE_KEY_PREFIX}{tenant_id}" for tenant_id in tenant_ids
        ]
        try:
